...
sudo docker run --restart unless-stopped --name Padma-API -p 5000:5000 --detach $NEW_IMAGE_ID;
...
sudo docker run --restart unless-stopped --name Padma-Frontend -p 8080:8080 --detach $NEW_IMAGE_ID;
